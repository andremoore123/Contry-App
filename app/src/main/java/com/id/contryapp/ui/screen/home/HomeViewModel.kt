    ...
    fun fetchCountries() {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            when (val response = repository.fetchCountries()) {
                is Resource.Error -> {
                    Log.d("HVM_ERROR", response.errorMessage)
                    handleState(HomeState.Error(response.errorMessage))
                }

                is Resource.Success -> {
                    Log.d("HVM_SUCCESS", response.data.toString())
                    handleState(
        ...
        viewModelScope.launch(Dispatchers.IO) {
            when (val currentState = _uiState.first().state) {
                is HomeState.Success -> {
                    val filteredData = currentState.data.filter { it.name.lowercase().contains(search.lowercase()) }
                    handleState(HomeState.Success(data = currentState.data, filteredData = if (search.isEmpty()) listOf() else filteredData))
                }
                else -> {}
            }
        }
